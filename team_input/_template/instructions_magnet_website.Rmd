...
- *Personal information.* All personal information (except for the picture) is stored in _index.md. Please update the following items only: (1) title, (2) role, (3) interests, (4) education, (5) social (remove the entries that are not relevant for you, e.g. linkedin), (6) cv (replace initials in filename), (7) biography (add your own). The files uses Markdown file which is a general markup language, which can be used to add layout text, add equations, etc For examples see [here](https://themes.gohugo.io//theme/academic/post/writing-technical-content/). For a cheatsheet with the main options see [here](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet).

- *Picture/avatar.* Save your picture as 'avatar.jpg'

- *CV.* Save your cv as '[lowercase initials].pdf'.

- *Publications.* We want to create one file with all MAGNET related publications, which can be uploaded to the website. Reference software such as Mendeley, Endnote or Zotero can save a reference list in bibtex (.bib) format. In case you use this type of software please save the reference list as [lowercase initials].bib (replace the initials). In case you are not using reference software, provide a Word file but, where possible, add the DOI of each publication. 
...
- Create a new subfolder using your full name (lowercase) as folder name in the `team` subfolder. Note that the name of this folder will be used to make automatic links with your name in the publications list. Please use the version you are use for most of your publications (e.g. full name without middle name initials).
